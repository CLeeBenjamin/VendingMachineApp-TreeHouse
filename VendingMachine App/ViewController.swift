...
//  VendingMachine App
//
//  Created by Caston  Boyd on 4/23/18.
...
class ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    @IBAction func purchase() {
        if let currentSelection = currentSelection {
            do {
                ...
                showAlertWith(title:"This item is out of stock", message: "Try another selection")
            } catch VendingMachineError.invalidSelection  {
                showAlertWith(title: "Invalid Selection", message: "Please make another selection")
            }catch VendingMachineError.insufficientFunds(required: let required){
    ...
    @IBAction func updateQuantity(_ sender: UIStepper) {
      let itemQuantity = Int(quantityStepper.value)
        updateDisplayWith(itemQuantity: itemQuantity)
